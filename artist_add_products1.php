...
if(isset($_POST['add_artwork']))
{
   ...
   else
   {
    ...
    {
        $res = mysqli_query($conn, "INSERT INTO `products`(user_id, name, artist_name, art_type, price, image, theme,length,breadth, description, placed_on) VALUES('$artist_id','$name', '$artist_name', '$type', '$price', '$image', '$theme', '$len', '$bre', '$des', '$date')");
        if($res)
        {
